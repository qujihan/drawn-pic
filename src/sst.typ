...
#let footer = draw-layout(
  ...
  // )[padding][#align(center)[complete to 40 bytes]],
...
#let block-arr-body = draw-layout(
  // debug:blue + 1pt,
  width: 100,
  ...
  title(side: "bottom", position: right + horizon)[
    s_b = shared_bytes \
    us_b = unsahred_bytes \
    v_l = value_length \
    key_d = key_delta \
  ...
  // restart num
  unit(25, fill: orange.lighten(70%), completion: true)[restart_num][#align(left)[ #sym.space #sym.arrow.r uint32_t #sym.space #sym.arrow.curve uint32_t ]],
...
#let data-block-info = draw-layout(
  // debug: red,
  width: 100,
  main-body-precent: 100,
  left-note-precent: 0,
  ...
  header(
    ...
    )[#align(center + horizon)[unsigned char[ ]]],

    header-unit(
      51,
      end-pos: 70,
    )[#align(center + horizon)[1 byte]],

    header-unit(
      71,
      end-pos: 100
    )[#align(center + horizon)[4 bytes]],
  ),
...
#let main-body = draw-layout(
  // debug: purple + 3pt,
  width: 30,
  main-body-precent: 15,
  left-note-precent: 0,
  right-note-precent: 70,
